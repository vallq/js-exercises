...
function filterUnderagedPeople(people, ageLimit) {
  const overAgedPeople = people.filter(({ age }) => age > ageLimit)
                                .map(({ name }) => name);
  return overAgedPeople;
}
...
function filterEmailsByDomain(domain, ...rest) {
  const returnEmails = [...rest].filter(email => email.includes(domain));
  return returnEmails;
}
